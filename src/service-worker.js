...
const CURRENT_CACHE_NAME = 'version-1.06';
let filesToCache = [
	'./',
	'./index.html',
	'./js/bundle.js',
...
self.addEventListener('install', event => {
    console.log("[ServiceWorker] Attempting to install service worker and cache files");
    event.waitUntil( addAllCacheFilesToCache(filesToCache, CURRENT_CACHE_NAME) );
});


async function addAllCacheFilesToCache(files, cacheName)
{
	let cache = await caches.open(cacheName);
	console.log("[ServiceWorker] Caching files to Cache");
...
self.addEventListener('activate', event => {
    ...
    event.waitUntil( makeCacheIsUpToDate(CURRENT_CACHE_NAME) );
});


async function makeCacheIsUpToDate(actualCacheName)
{
	let cacheNames = await caches.keys();
	let cacheNamesToDelete = cacheNames.filter( cacheName => cacheName !== actualCacheName);
	cacheNamesToDelete.forEach( cacheName => deleteCachedFiles(cacheName) );

	return Promise.all( cacheNamesToDelete );
}
...
self.addEventListener('fetch', event => {
   ...
   event.respondWith( getRequestedData(event.request, CURRENT_CACHE_NAME) ) ;
});


async function getRequestedData(request, cacheName)
{
		let foundInCache = await getFromCache(request);
...
{
		console.log("[ServiceWorker] Found in Cache ", request.url, result);
		return result;
}
...
{
		console.log("[ServiceWorker] Not found in Cache, atempting to fetch data from location ", request.url);
		let requestClone = request.clone();
		let serverResponse = await fetch(requestClone) ;
		let serverResponseClone = serverResponse.clone();

		addNewDataToCache(cacheName, request, serverResponseClone);
...
async function addNewDataToCache(cacheName, request, data)
{
		...
		console.log("[ServiceWorker] New data has been successfully cached ", request.url);
}
